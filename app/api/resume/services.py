...
# This avoids reloading models for every request.
reader = easyocr.Reader(['en'])

# Initialize a global thread pool for running synchronous OCR and LLM calls.
# This ensures heavy, CPU-bound tasks don't block FastAPI's main event loop,
# keeping the API responsive. Adjust max_workers based on your CPU cores.
    ...
    try:
        response = await loop.run_in_executor(
            ...
            lambda: azure_openai_client.chat.completions.create(
                ...
                max_tokens=2500  # Generous token limit for detailed resume data
            )
        )
        ...
        # Initialize with the full schema to ensure all keys are always present
        parsed_resume = {
            "basic_details": {
                "name": "N/A", "email": "N/A", "phone": "N/A",
                "links": {} # Initialize links as an empty dict
            },
            "technical_expertise": [],
            "certifications": [],
        ...
        if "basic_details" in llm_data and isinstance(llm_data["basic_details"], dict):
            ...
            # Handle links sub-dictionary within basic_details
            if "links" in llm_data["basic_details"] and isinstance(llm_data["basic_details"]["links"], dict):
                for link_key, link_val in llm_data["basic_details"]["links"].items():
                    if link_val and str(link_val).strip() != "N/A": # Ensure valid links are added
                        parsed_resume["basic_details"]["links"][link_key.lower()] = str(link_val).strip()

        # Populate technical_expertise
        if "technical_expertise" in llm_data and isinstance(llm_data["technical_expertise"], list):
            ...
            ]))) # Ensure unique, cleaned, and sorted skills
        ...
        if "certifications" in llm_data and isinstance(llm_data["certifications"], list):
            ...
            for cert_item in llm_data["certifications"]:
                if isinstance(cert_item, dict):
                    ...
                    if title and title != "N/A": # Only add if title is meaningful
                        certs.append({"title": str(title).strip(), "date": str(date).strip()})
                elif isinstance(cert_item, str) and str(cert_item).strip() != "N/A": # Handle cases where LLM might just output a string title
                    certs.append({"title": str(cert_item).strip(), "date": "N/A"})
            # Remove duplicates based on title (case-insensitive) and sort for consistent output
            seen_titles = set()
            unique_certs = []
            for cert in certs:
            ...
            for exp in llm_data["professional_experience"]:
                if isinstance(exp, dict):
                    ...
                    if isinstance(responsibilities, str):
                        responsibilities = [responsibilities] # Convert single string to list
                    ...
                    experiences.append({
                        ...
                        "responsibilities": [str(r).strip() for r in responsibilities if str(r).strip() != "N/A"] # Ensure clean strings
                    })
        
        # Sort professional experience by length of responsibilities in descending order
        ...
        # Return a default empty structure aligned with the schema on JSON parsing failure
        return {
        ...
        # Return a default empty structure for other errors
        return {
            "basic_details": {"name": "N/A", "email": "N/A", "phone": "N/A", "links": {}},
            "technical_expertise": [],
            "certifications": [],
...
async def process_resume_pdf(pdf_bytes: bytes) -> dict:
    """
    Handles the entire pipeline of processing a PDF resume:
    - Extracts native text.
    - Performs OCR on embedded images.
    - Conditionally performs full-page OCR.
    ...
    for page_num in range(len(doc)):
        ...
        # 1. Extract text from the PDF's native text layer (always perform this first)
        ...
        # Decide whether to perform full-page OCR based on native text content.
        # This is a heuristic to save time if a page is already mostly searchable text.
        perform_full_page_ocr = False
        native_text_len = len(text_from_pdf_layer.strip())

        if native_text_len < 100: # If very little native text, OCR the whole page
            perform_full_page_ocr = True
        else:
            # If native text exists, check for common resume sections.
            # If these are absent, the page might still be a scan or poorly formatted.
        ...
        # --- Embedded Image OCR ---
        # Always attempt OCR on embedded images, as they often contain critical information
        # like logos or badges that native text extraction misses.
        ...
        if perform_full_page_ocr:
            try:
                # Render at 1x resolution (matrix=fitz.Matrix(1, 1)) for CPU performance.
                # Higher resolutions significantly increase processing time.
    ...
    # Concurrently await all OCR tasks to maximize throughput.
    ...
    # Combine all extracted text (native + OCR) into one large string for the LLM.
    full_resume_text = "\n".join(all_extracted_text_segments)

    print("Calling LLM for full resume parsing...")
    # Call the LLM to get the structured data from the combined text.
    ...
    # Try to extract the name from the parsed data for a meaningful filename.
    extracted_name = parsed_resume_data.get("basic_details", {}).get("name", "").strip()
    if extracted_name and extracted_name != "N/A":
        # Sanitize the name for use in a filename (remove special chars, replace spaces with underscores).
        sanitized_name = re.sub(r'[^\w\s-]', '', extracted_name).strip().replace(' ', '_')
        output_filename = f"{sanitized_name}_resume.json"
    else:
        # Fallback to a generic filename if a name couldn't be extracted.
        output_filename = f"parsed_resume.json" # Default to generic if name is N/A or empty
